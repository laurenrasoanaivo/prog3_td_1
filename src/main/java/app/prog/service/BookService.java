...
public class BookService {
    ...
    public List<Book> createBooks(List<Book> toCreate) {
        for(int i=0; i<toCreate.size(); i++){
            toCreate.get(i).setId(getBooks().size()+i+1);
            toCreate.get(i).setPageNumber(0);
        }
    ...
    public List<Book> updateBooks(List<Book> toUpdate) {
        for(int i=0; i<toUpdate.size(); i++){
            Optional<Book> optional = repository.findById(String.valueOf(toUpdate.get(i).getId()));
            if (optional.isPresent()) {
                toUpdate.get(i).setPageNumber(optional.get().getPageNumber());
            } else {
                toUpdate.get(i).setPageNumber(0);
            }
        }
